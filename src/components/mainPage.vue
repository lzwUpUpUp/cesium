      ...
      <div class="middle" ref="middle">
        ...
        <div class="middleBottom-Bar" ref="middleBottomBar">
          ...
          >
            ...
            <P>{{item.SensorType}}</P>
            <P>{{Number(item.Lon).toFixed(3)}}°E/{{Number(item.Lat).toFixed(3)}}°N</P>
            <P>{{Number(item.Hei).toFixed(0)}}</P>
            <P>IFF:{{item.IFF}}</P>
          </div>
        </div>
      </div>
              ...
              <div class="platformSensorBottom">
                ...
                <ul class="platFormBottomSelectDeteil">
                  <li
                    ...
                    :key="index"
                    :class="{sensorLight:item.SensorState == 1}"
                    @dblclick="issuedOrder(item)"
                  >
                    <p>
                      <span>{{item.SensorStateText}}</span>&nbsp;
      ...
      this.$axios({
        ...
        for (var i = 0; i < res.data.data.sensors.length; i++) {
          if (res.data.data.sensors[i].SensorState == 0) {
            res.data.data.sensors[i].SensorStateText = '失效'
          } else if (res.data.data.sensors[i].SensorState == 1) {
            res.data.data.sensors[i].SensorStateText = '正常'
          } else if (res.data.data.sensors[i].SensorState == 2) {
            res.data.data.sensors[i].SensorStateText = '降级'
          } else if (res.data.data.sensors[i].SensorState == 3) {
            res.data.data.sensors[i].SensorStateText = '拒绝'
          }
        }
    ...
    issuedAnOrder(data) {
      this.$axios({
        ...
        if (res.data.status == 1) {
          ...
          // 通信类传感器弹窗数据
          this.communicationId = '', //目标Id
            document.getElementById('communicationTime').value = '', //开始收集时间
            this.coDurationTime = '', //持续时间
            this.coOffsetTime = '', //偏差时间
            this.coLocation = '', //定位收集标志
            this.coSide = '', //测向收集标志
            this.coHistory = '', //历史收集标志
            this.coRecognize = "", //识别收集标志
            this.coCenter = '', //中心频率
            this.coRandomRrror = '',// 频率随机误差sigma
            this.coReceiver = '', //接收机的宽带
            this.coModulation = "", //调制方式
            this.coAoi = '',//AOI/SOI,
            this.coSignal = 0, //信号放大
            this.coNodeNum = '', //节点个数
            this.coLon = '', //目标经度
            this.coLat = "", //目标纬度
            this.coHigh = '',//目标高度
            ...
            this.coLong = '', //误差椭圆长轴
            this.coAngle = '', //误差椭圆倾角：
            this.coActionMark = 0 //动作标识


          // 电子类传感器弹窗数据
          this.elId = '', //目标Id
            document.getElementById('electronicTime').value = '', //开始收集时间
            this.elDurationTime = '', //持续时间
            this.elOffsetTime = '', //偏差时间
            this.elLocation = '', //定位收集标志
            this.elSide = '', //测向收集标志
            this.elHistory = '', //历史收集标志
            this.elRecognize = "", //识别收集标志
            this.elCenter = '', //中心频率
            this.elLows = '', //低频测量下限频率值
            this.elFrequency = '', //高频测量下限频率值
            this.elSpecification = '', //电子信号规格号
            this.elConfidence = '', //电子信号的可信级别
            this.elRandomRrror = '',// 频率测量的1倍误差sigma估计值
            this.elReceiver = '', //接收机的宽带
            this.elModulation = "", //调制方式
            this.elAoi = '',//AOI/SOI,
            this.elSignal = 0, //信号放大
            this.elWidth = '', //脉冲宽度
            this.elInterval = '', //脉冲间隔
            this.elLon = '', //目标经度
            this.elLat = "", //目标纬度
            this.elHigh = '',//目标高度
            this.elShort = '', //误差椭圆短轴
            this.elLong = '', //误差椭圆长轴
            this.elAngle = '', //误差椭圆倾角：
            this.elActionMark = 0 //动作标识



          // 图像类传感器弹窗数据
          this.piId = '', //目标Id
            document.getElementById('pictureTime').value = '', //开始收集时间
            this.piDurationTime = '', //持续时间
            this.piOffsetTime = '', //偏差时间
            this.piLocation = '', //定位收集标志
            this.piHistory = '', //历史收集标志
            this.piCollectMark = "", //识别收集标志
            this.piSignal = '', //电子信号规格号
            this.piConfidence = '', //电子信号的可信级别
            this.piNation = '', //国家
            this.piAoi = '',//AOI/SOI,
            this.piAoiLeval = '',//AOI/SOI可信识别,
            this.piEnviionment = 0, //环境
            this.piRecognize = 0, //识别
            this.piLon = '', //目标经度
            this.piLat = "", //目标纬度
            this.piHigh = '',//目标高度
            this.piShort = '', //误差椭圆短轴
            this.piLong = '', //误差椭圆长轴
            this.piAngle = '', //误差椭圆倾角：
            this.piGiveUp = '', //放弃追踪标志
            this.piSpeed = '', //速度
            this.piHeading = '', //航向
            this.piActionMark = 0 //动作标识



          // 对海雷达类传感器弹窗数据
          this.seaId = '', //目标Id
            document.getElementById('forSeaTime').value = '', //开始收集时间
            this.seaDurationTime = '', //持续时间
            this.seaOffsetTime = '', //偏差时间
            this.seaLocation = '', //定位收集标志
            this.seaHistory = '', //历史收集标志
            this.seaLon = '', //目标经度
            this.seaLat = "", //目标纬度
            this.seaHigh = '',//目标高度
            this.seaShort = '', //误差椭圆短轴
            this.seaLong = '', //误差椭圆长轴
            this.seaAngle = '',//误差椭圆倾角
            this.seaTargetType = 0, //目标分类 select
            this.seaSignal = '', //电子信号规格号
            this.seaConfidence = '', //电子信号的可信级别
            this.seaTransmitter = '', //发射机扫描频率
            this.seaAoi = '',//AOI/SOI,
            this.seaAoiLeval = '',//AOI/SOI可信识别,
            document.getElementById('airFirstTime').value = '', //首次发现时间
            this.seaFirstLon = '', //首次发现目标经度
            this.seaFirstLat = '', //首次发现目标纬度
            this.seaFirstHight = '', //首次发现目标高度
            this.seaSpeed = '', //速度
            this.seaHeading = '', //航向
            this.seaTargetWidth = '', //目标宽度
            this.seaLength = '', //目标长度
            this.seaActionMark = 0 //动作标识



          // 对空雷达类传感器弹窗数据
          this.airId = '', //目标Id
            document.getElementById('forAirTime').value = '', //开始收集时间
            this.airDurationTime = '', //持续时间
            this.airOffsetTime = '', //偏差时间
            this.airLocation = '', //定位收集标志
            this.airHistory = '', //历史收集标志
            this.airLon = '', //目标经度
            this.airLat = "", //目标纬度
            this.airHigh = '',//目标高度
            this.airShort = '', //误差椭圆短轴
            this.airLong = '', //误差椭圆长轴
            this.airAngle = '',//误差椭圆倾角
            this.airTargetType = 0, //目标分类 select
            this.airSignal = '', //电子信号规格号
            this.airConfidence = '', //电子信号的可信级别
            this.airTransmitter = '', //发射机扫描频率
            this.airAoi = '',//AOI/SOI,
            this.airAoiLeval = '',//AOI/SOI可信识别,
            document.getElementById('airFirstTime').value = '', //首次发现时间
            this.airFirstLon = '', //首次发现目标经度
            this.airFirstLat = '', //首次发现目标纬度
            this.airFirstHight = '', //首次发现目标高度
            this.airSpeed = '', //速度
            this.airHeading = '', //航向
            this.airTargetWidth = '', //目标宽度
            this.airLength = '', //目标长度
            this.airActionMark = 0//动作标识
        }
      });
    },
